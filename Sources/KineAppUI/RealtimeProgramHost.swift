...
public struct RealtimeProgramHostView: NSViewRepresentable {
    ...
    public final class Coordinator {
        ...
        init(workspace: WorkspaceModel) {
            ...
            guard let d = KineRender.device else {
        ...
        private func installDisplayLink() {
            ...
            guard let link else {
                KineDebugLog.log("[Realtime] CVDisplayLinkCreate failed")
                return
            }
            ...
            Task.detached(priority: .userInitiated) { [weak self] in
                ...
                if let reader {
                    ...
                    do {
                        ...
                        KineDebugLog.log("[Realtime] composeAsync failed: \(error.localizedDescription)")
                    }
                    drawable.present()
                } else {
                    drawable.present()
...
public final class CacheFrameReader: @unchecked Sendable {
    ...
    public init(url: URL, device: MTLDevice) throws {
        ...
        guard let t = asset.tracks(withMediaType: .video).first else {
            throw NSError(domain: "kine.cache", code: 1)
        }
        self.track = t
