...
private struct FilmstripClipRow: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 5)
        .background(isActive ? KineTheme.accent.opacity(0.10) : Color.clear)
        .onAppear {
        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                ...
                if isActive, let band = selectionBand(width: w) {
                    Rectangle()
                        .fill(KineTheme.accent.opacity(0.22))
                        .frame(width: band.width, height: geo.size.height)
                        .offset(x: band.x, y: 0)
                        .overlay(alignment: .leading) {
                            Rectangle().fill(KineTheme.accent).frame(width: 1.5)
                                .offset(x: band.x)
                        }
                }
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 4, style: .continuous)
                    .stroke(isActive ? KineTheme.accent : Color.black.opacity(0.4),
...
private struct WaveformStrip: View {
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            ctx.stroke(path, with: .color(KineTheme.accent.opacity(0.7)), lineWidth: 1)
        }
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: "rectangle.stack.fill")
                .frame(width: 18)
                .foregroundStyle(isActive ? KineTheme.accent : .secondary)
        ...
        .padding(.horizontal, 12)
        .padding(.vertical, 4)
        .background(isActive ? KineTheme.accent.opacity(0.22) : Color.clear)
    }
