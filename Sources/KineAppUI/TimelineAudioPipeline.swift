...
/// Bridges Kine's timeline data model to Polymerge's
...
public final class TimelineAudioPipeline {
    ...
    /// Map from engine TrackBuffer.id → Kine audio track index, so
    ...
    public func sync(to sequence: Sequence, mediaPool: MediaPool, startPlayheadSeconds: Double) async {
        ...
        if signature != lastBuiltSignature {
            ...
            KineDebugLog.log("[TimelineAudio] engine.prepare: tracks=\(buffers.count) totalFrames=\(total) (=\(Double(total)/Self.sampleRate)s) initialSample=\(initialSample)")
    ...
    /// given Kine audio track index when last building.
    ...
    private func buildBuffer(
        ...
        // Decode ONLY this clip's span (zero-padded).
        ...
        do {
            ...
            KineDebugLog.log("[TimelineAudio] decode failed for \(source.name): \(error)")
            return nil
        }
        guard let firstCh = rawChannels.first, !firstCh.isEmpty else { return nil }
        let channelCount = rawChannels.count

        KineDebugLog.log("[TimelineAudio] \(source.name): srcStart=\(inFrame) len=\(sliceLength) ch=\(channelCount) timeline=[\(placed.timelineRange.start.seconds - extLeftSec), \(placed.timelineRange.end.seconds + extRightSec))")
