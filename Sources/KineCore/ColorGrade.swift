...
/// Per-clip color grade — the typed view of the `kine.color` effect that
...
/// the LUT path are NOT keyframable (stored on the same `kine.color`
...
public enum ColorGradeParameter: String, CaseIterable, Sendable {
    ...
    public var effectKey: String { "kine.color" }
...
public extension PlacedClip {
    ...
    func colorGrade(at clipLocalSeconds: Double) -> ColorGrade {
        ...
        for eff in effects where !eff.isBypassed && eff.effectKey == "kine.color" {
    ...
    func hasKeyframes(for parameter: ColorGradeParameter) -> Bool {
        for eff in effects where eff.effectKey == "kine.color" {
    ...
    /// Index of the `kine.color` effect, creating one if needed.
    private mutating func colorEffectIndex() -> Int {
        if let i = effects.firstIndex(where: { $0.effectKey == "kine.color" }) { return i }
        effects.append(EffectInstance(effectKey: "kine.color", parameters: [:]))
    ...
    mutating func removeColorKeyframe(_ parameter: ColorGradeParameter, at clipLocalTime: Double, tolerance: Double = 0.01) {
        guard let i = effects.firstIndex(where: { $0.effectKey == "kine.color" }),
