...
/// Per-pane configuration for `PreemSplitView`.
///
...
public struct PreemPaneSpec: Sendable {
...
public struct PreemSplitView<First: View, Second: View>: NSViewControllerRepresentable {
    public let isVertical: Bool          // true → horizontal stack (vertical divider)
    public let autosaveName: String
    public let firstSpec: PreemPaneSpec
    public let secondSpec: PreemPaneSpec
    ...
    public init(
        isVertical: Bool,
        autosaveName: String,
        firstSpec: PreemPaneSpec,
        secondSpec: PreemPaneSpec,
