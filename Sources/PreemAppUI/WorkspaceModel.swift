...
public final class WorkspaceModel: ObservableObject {
    ...
    /// the ⌘+ / ⌘- shortcuts. Pushed into `PreemTimelineView` each
    /// `updateNSView` cycle.
    ...
    public func pushUndoSnapshot() {
        ...
        do {
            ...
            PreemDebugLog.log("[Undo] snapshot encode failed: \(error.localizedDescription)")
        }
    }
    ...
    public func undo() {
        ...
        do {
            ...
            PreemDebugLog.log("[Undo] restore failed: \(error.localizedDescription)")
        }
    }
    ...
    public func redo() {
        ...
        do {
            ...
            PreemDebugLog.log("[Redo] restore failed: \(error.localizedDescription)")
        }
    }
    ...
    private let slateOCR = SlateOCR()
    private let shotClassifier = ShotClassifier()
    ...
    /// when the only WorkspaceModel — owned by `PreemRootView` —
    ...
    public func openProject() {
        ...
        panel.title = "Open Preem Project"
        panel.begin { [weak self] response in
    ...
    private func performSave(to url: URL) {
        do {
            ...
            PreemDebugLog.log("[Workspace] save failed: \(error.localizedDescription)")
        }
    }
    ...
    private func performOpen(url: URL) {
        do {
            ...
            saveError = error.localizedDescription
            PreemDebugLog.log("[Workspace] open failed: \(error.localizedDescription)")
        }
    }
        ...
        Task.detached { [weak self] in
            do {
                ...
                await MainActor.run { [weak self] in
                    ...
                    PreemDebugLog.log("[Render] failed: \(error.localizedDescription)")
                }
            }
        }
    }
        ...
        Task.detached { [weak self] in
            do {
                ...
                await MainActor.run { [weak self] in
                    ...
                    self.lastRenderURL = outputURL
                    self.activeEncoder = nil
                    PreemDebugLog.log("[Export] wrote \(outputURL.path)")
                }
            } catch {
                await MainActor.run { [weak self] in
                    guard let self else { return }
                    ...
                    self.activeEncoder = nil
                    self.renderError = error.localizedDescription
                    PreemDebugLog.log("[Export] failed: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    /// from PreemSettings; clamped to half the clip's own length;
    ...
    public func applyFadeAtEdge(_ edge: ClipEdgeSelection, durationSeconds: Double? = nil) {
        let kind = PreemSettings.shared.defaultTransitionKind
        let rawTotal = durationSeconds ?? PreemSettings.shared.defaultTransitionSeconds(for: activeSequence)
    ...
    public func addCrossDissolveAtPlayhead(totalDuration: Double? = nil) {
        let duration = totalDuration ?? PreemSettings.shared.defaultTransitionSeconds(for: activeSequence)
        let kind = PreemSettings.shared.defaultTransitionKind
    ...
    public func applyTransitionAtPlayhead(kind: String, totalDuration: Double) -> Bool {
        ...
        if !didApply {
            PreemDebugLog.log("[Transitions] no abutting cut near playhead (t=\(t)) — nothing to add")
        }
        return didApply
    }
    ...
    public func applyTransitionAtCut(_ cut: CutSelection, kind: String? = nil, leftHalf: Double? = nil, rightHalf: Double? = nil) {
        let chosenKind = kind ?? PreemSettings.shared.defaultTransitionKind
        let half = (leftHalf ?? PreemSettings.shared.defaultTransitionSeconds(for: activeSequence) / 2)
    ...
    public func resizeTransition(at cut: CutSelection, leftHalf: Double? = nil, rightHalf: Double? = nil) {
        let isVideo = cut.trackKind == 0
        let fallbackKind = PreemSettings.shared.defaultTransitionKind
    ...
    private func ingestSingle(url: URL) async {
        ...
        do {
            ...
            if !clip.videoTracks.isEmpty || !clip.audioTracks.isEmpty {
                scheduleML(for: clip)
            }
            schedulePreviews(for: clip)
        } catch {
    ...
    private func isProbablyMediaFile(_ url: URL) -> Bool {
        ...
        return ["mov", "mp4", "m4v", "mxf", "wav", "aif", "aiff", "mp3", "mkv"].contains(ext)
    }

    // MARK: - ML pipeline scheduling

    private func scheduleML(for clip: ClipSource) {
        guard !mlInFlight.contains(clip.id) else { return }
        mlInFlight.insert(clip.id)

        let clipID = clip.id
        let ocr = slateOCR
        let classifier = shotClassifier
        let transcribe = transcriber
        let snapshot = clip

        Task.detached(priority: .utility) {
            async let slateResult: SlateData? = {
                do { return try await ocr.recognize(clip: snapshot) }
                catch { return nil }
            }()

            async let shotResult: ShotType = {
                do { return try await classifier.classify(clip: snapshot) }
                catch { return .unknown }
            }()

            async let transcriptResult: TranscriptData? = {
                do { return try await transcribe.transcribe(clip: snapshot) }
                catch { return nil }
            }()

            let slate      = await slateResult
            let shot       = await shotResult
            let transcript = await transcriptResult

            await MainActor.run {
                self.applySlate(slate, to: clipID)
                self.applyShot(shot, to: clipID)
                self.applyTranscript(transcript, to: clipID)
                self.mlInFlight.remove(clipID)
            }
        }
    }

    private func applySlate(_ slate: SlateData?, to clipID: ClipID) {
        guard let slate else { return }
        guard var clip = project.mediaPool.clips[clipID] else { return }
        clip.ml.slate = slate
        if clip.scene == nil { clip.scene = slate.scene }
        if clip.take == nil { clip.take = slate.take }
        if clip.roll == nil { clip.roll = slate.roll }
        project.mediaPool.clips[clipID] = clip
        project.modifiedAt = Date()

        if sourceClip?.id == clipID { sourceClip = clip }
    }

    private func applyShot(_ shot: ShotType, to clipID: ClipID) {
        guard var clip = project.mediaPool.clips[clipID] else { return }
        clip.ml.shotType = shot
        project.mediaPool.clips[clipID] = clip
        project.modifiedAt = Date()
        if sourceClip?.id == clipID { sourceClip = clip }
    }

    private func applyTranscript(_ transcript: TranscriptData?, to clipID: ClipID) {
        guard let transcript else { return }
        guard var clip = project.mediaPool.clips[clipID] else { return }
        clip.ml.transcript = transcript
        project.mediaPool.clips[clipID] = clip
        project.modifiedAt = Date()
        if sourceClip?.id == clipID { sourceClip = clip }
    }
