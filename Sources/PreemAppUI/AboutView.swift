...
/// Branded About panel. Draws Preem's mark (timeline-track bars — the
        ...
        VStack(spacing: 16) {
            ...
            VStack(spacing: 4) {
                Text("Preem")
                    ...
                    .foregroundStyle(PreemTheme.text)
            ...
            VStack(spacing: 2) {
                Text("A Polymerge sibling")
                    .font(.system(size: 11))
                    .foregroundStyle(PreemTheme.accent)
                Text("Built on the Polymerge engine")
                    .font(.system(size: 10))
                    .foregroundStyle(PreemTheme.textDim)
            }
    ...
    private var mark: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(
                    LinearGradient(colors: [PreemTheme.bgCard, PreemTheme.bg],
                                   startPoint: .topLeading, endPoint: .bottomTrailing)
                )
                .overlay(RoundedRectangle(cornerRadius: 20, style: .continuous)
                    .strokeBorder(PreemTheme.border, lineWidth: 1))
            ...
            GeometryReader { geo in
                ...
                VStack(spacing: 0) {
                    ForEach(0..<n, id: \.self) { i in
                        Capsule()
                            .fill(LinearGradient(colors: [PreemTheme.accent, Color(hex: "FFC46B")],
                                                 startPoint: .leading, endPoint: .trailing))
                            .frame(width: w * barWidths[i], height: barH)
                            .shadow(color: PreemTheme.accentGlow, radius: 4)
