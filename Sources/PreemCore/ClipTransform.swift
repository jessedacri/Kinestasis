...
/// effect instances (`preem.transform` and `preem.crop`); this struct
...
public extension PlacedClip {
    ...
    /// any existing `preem.transform` / `preem.crop` instances with
    ...
    mutating func setTransform(_ transform: ClipTransform) {
        effects.removeAll { $0.effectKey == "preem.transform" || $0.effectKey == "preem.crop" }
        if transform == .identity { return }

        // preem.transform — only write if something differs from identity
        let needsTransform =
